...
def test_anonymous_file_open_fail():
    class FakeLib:
        pass

    lib = FakeLib()
    lib.os_create_anonymous_file = lambda size: -1
    af = AnonymousFile(2**30)
    with pytest.raises(OSError, match="Unable to create anonymous file"):
        af.open()
